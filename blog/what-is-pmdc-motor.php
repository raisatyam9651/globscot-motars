...
<p>The operation of a PMDC motor is governed by <strong>Lorentz's Force Law</strong> and <strong>Faraday's Law of Induction</strong>. When an external DC voltage source (<b>V</b>) is applied across the brush terminals, electrical current (<b>I<sub>a</sub></b>) flows into the armature winding situated within the stator's magnetic field (<b>B</b>).</p>

<p>Every current-carrying conductor inside the magnetic field experiences a mechanical force (<b>F</b>) defined by the vector equation:</p>

<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">F = I<sub>a</sub> × (L × B)</div>

<p>Where <b>I<sub>a</sub></b> is armature current, <b>L</b> is the active length of the conductor, and <b>B</b> is the magnetic flux density. Because conductors on opposite sides of the rotor core carry current in opposing relative directions, equal and opposite forces create a mechanical torque (<b>τ</b>) about the shaft axis:</p>

<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">τ = K<sub>t</sub> × Φ × I<sub>a</sub></div>

<p>Where <b>K<sub>t</sub></b> is the motor torque constant and <b>Φ</b> is the fixed magnetic flux generated by the permanent magnets. As the rotor turns, the mechanical action of the commutator automatically switches current direction in each coil as it crosses the magnetic neutral axis. This mechanical commutation ensures continuous, uni-directional electromagnetic torque generation.</p>

<p>Simultaneously, as the armature conductors cut the permanent magnetic field, a counter-electromotive force (Back EMF, <b>E<sub>b</sub></b>) is induced within the rotor windings opposing the supply voltage:</p>

<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">E<sub>b</sub> = K<sub>e</sub> × Φ × ω</div>

<p>Where <b>K<sub>e</sub></b> is the back-EMF constant and <b>ω</b> is rotational angular velocity in rad/s. The terminal voltage equation of a PMDC motor is expressed as:</p>

<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">V = E<sub>b</sub> + I<sub>a</sub>R<sub>a</sub> = (K<sub>e</sub>Φω) + I<sub>a</sub>R<sub>a</sub></div>

<p>Where <b>R<sub>a</sub></b> represents the total armature resistance including brush contact drops. For a deeper breakdown of basic motor principles, read our detailed comparison on the <a href="/benefits-of-using-dc-motors/">key benefits of using DC motors</a> in industrial automation.</p>
...
<td>$ω = \frac{V}{K_e Φ} - \frac{R_a}{(K_e Φ)(K_t Φ)} × τ$</td>
<td>Delivers a straight-line dropping speed curve; speed decreases predictably as mechanical shaft load increases.</td>
</tr>
<tr>
<td><strong>Stall Torque (<b>τ<sub>stall</sub></b>)</strong></td>
<td>$τ_{stall} = K_t × Φ × ≤ft(\frac{V}{R_a}\right)$</td>
<td>Maximum starting torque available at zero speed ($ω = 0$); dictates acceleration under heavy inertia.</td>
</tr>
<tr>
<td><strong>No-Load Speed (<b>ω<sub>0</sub></b>)</strong></td>
<td>$ω_0 = \frac{V - I_{0} R_a}{K_e Φ}$</td>
<td>Maximum shaft rotation speed when no external load torque is applied.</td>
</tr>
<tr>
<td><strong>Motor Efficiency (<b>η</b>)</strong></td>
<td>$η = \frac{P_{out}}{P_{in}} = \frac{τ × ω}{V × I_{total}}$</td>
...
<li><strong>Superior Operating Efficiency:</strong> Because permanent magnets require zero electrical power to maintain field flux, internal thermal losses (<b>I<sup>2</sup>R</b>) are limited strictly to the rotor windings.</li>
...
<p>Calculate continuous load torque (<b>τ<sub>cont</sub></b>), peak acceleration torque (<b>τ<sub>peak</sub></b>), and operating speed (<b>ω</b>) using basic dynamics:</p>

<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">τ<sub>load</sub> = τ<sub>friction</sub> + τ<sub>process</sub> + J<sub>total</sub> × α</div>

<p>Where <b>J<sub>total</sub></b> is total system inertia reflected to the motor shaft and <b>α</b> is rotational angular acceleration (rad/s<sup>2</sup>). Ensure that <b>τ<sub>cont</sub></b> lies well within the motor's Continuous Operating Zone on its thermal curve.</p>
...
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">τ<sub>rms</sub> = √( Σ(τ<sub>i</sub><sup>2</sup> × t<sub>i</sub>) / Σ t<sub>i</sub> )</div>

<p>Select a motor whose continuous torque rating exceeds <b>τ<sub>rms</sub></b> to avoid thermal breakdown of winding insulation.</p>

<h3>Step 4: Check Stall Current & Power Supply Limits</h3>
<p>Ensure your DC power supply and electronic motor driver (H-Bridge PWM driver) can safely deliver peak stall current (I<sub>stall</sub> = V / R<sub>a</sub>) without triggering overcurrent shutdown or severe voltage sag.</p>
...
<ul>
<li><strong>Robot Total Mass (<b>m</b>):</strong> 40 kg (2-wheel drive, 20 kg per wheel)</li>
<li><strong>Wheel Radius (<b>r</b>):</strong> 0.075 m (75 mm)</li>
<li><strong>Maximum Speed (<b>v</b>):</strong> 1.5 m/s</li>
<li><strong>Acceleration Time (<b>t<sub>acc</sub></b>):</strong> 1.0 s to reach max speed</li>
<li><strong>Rolling Resistance Coefficient (<b>μ</b>):</strong> 0.03</li>
<li><strong>Available Battery Supply (<b>V</b>):</strong> 24V DC</li>
</ul>

<h3>Calculation Steps:</h3>
<ol>
<li><strong>Wheel Rotational Speed:</strong>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">ω = v / r = 1.5 / 0.075 = 20 rad/s ≈ 191 RPM</div>
</li>
<li><strong>Rolling Friction Resistance Force per wheel:</strong>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">F<sub>friction</sub> = μ × (m<sub>wheel</sub> × g) = 0.03 × (20 kg × 9.81 m/s<sup>2</sup>) = 5.886 N</div>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">τ<sub>friction</sub> = F<sub>friction</sub> × r = 5.886 × 0.075 = 0.441 N·m</div>
</li>
<li><strong>Acceleration Force & Torque per wheel:</strong>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">a = Δv / Δt = 1.5 / 1.0 = 1.5 m/s<sup>2</sup></div>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">F<sub>acc</sub> = m<sub>wheel</sub> × a = 20 × 1.5 = 30 N</div>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">τ<sub>acc</sub> = F<sub>acc</sub> × r = 30 × 0.075 = 2.25 N·m</div>
</li>
<li><strong>Peak Starting Torque Required:</strong>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">τ<sub>peak</sub> = τ<sub>friction</sub> + τ<sub>acc</sub> = 0.441 + 2.25 = 2.691 N·m</div>
</li>
<li><strong>Continuous Shaft Mechanical Output Power:</strong>
<div class="alert alert-light border text-center font-weight-bold p-3 my-3" style="font-size: 1.1rem; background-color: #fff9ed; border-color: #eb9600 !important; color: #222;">P<sub>mech</sub> = τ<sub>friction</sub> × ω = 0.441 N·m × 20 rad/s = 8.82 Watts</div>
</li>
</ol>

<p><strong>Selection Conclusion:</strong> A direct-drive PMDC motor requiring 2.69 N·m at 191 RPM would be unnecessarily large. By pairing a 24V PMDC motor (rated at 3000 RPM, 0.15 N·m continuous torque) with a 15:1 planetary gearbox, the output speed becomes $3000 / 15 = 200 RPM$ and continuous output torque multiplies to $0.15 × 15 × 0.85 = 1.91 \text{ N}×\text{m}$, easily meeting continuous and acceleration demands. For maintenance-free high-speed alternatives, evaluate our <a href="/what-is-bldc-motor/">working principle of BLDC motors</a> guide or explore our <a href="/products-bldc-motors/">brushless DC (BLDC) motor series</a>.</p>
...
<p>Yes. Because the stator field flux (<b>Φ</b>) is fixed by permanent magnets, reversing the polarity of the applied DC supply voltage at the motor terminals reverses the direction of armature current (<b>I<sub>a</sub></b>), causing the Lorentz force vector and rotational direction to flip instantly.</p>
...
<p>Severe thermal overload ($>150^\circ\text{C}$) can cause irreversible demagnetization of permanent magnets (especially lower-grade Ferrite or NdFeB magnets exceeding their Curie temperature threshold), permanently reducing motor torque constant (<b>K<sub>t</sub></b>) and destroying armature insulation.</p>
...
<li><a href="/advantages-of-using-bldc-motor-for-micro-and-macro-applications/">Advantages of using bldc motor for micro and macro applications.</a></li>
<li><a href="/what-is-bldc-motor/">What is bldc motor?</a></li>
<li><a href="/dc-motor-maintenance/">DC motor maintenance.</a></li>
</ul>
</div>
